...
{address_hints}
请按真人阅读方式判断每句对话的说话人：
1. 先读旁白，确定这一段【在场人物】及其【登场顺序】——留意“A与B / A和B / A、B 走了过来”、“X介绍说有甲乙丙”这类引入，先出现/先被点名者通常先开口。
2. 用登场顺序【锚定第一句】说话人，定下交替基准。⚠️前文若已明确给出登场/介绍顺序，必须按该顺序分配，不要随机；只有完全没有顺序线索时才默认交替。
...
5. 【场景一致性】没有旁白明确写某人加入或离开时，说话人集合应保持稳定、按既定轮次进行；⚠️不要引入旁白未提及、明显不在本场景的角色（宁可保留某个已在场角色，也不要凭空换成无关角色）。
6. 第一人称“我”说出口的话归叙述者 {narrator}。
7. 无名群体（如一群摄影师、一群女生、起哄的众人）统一用「群众·<群体名>」（例：群众·摄影师们、群众·女性们）；单个无名路人用「其他」；若某【{prefix}】其实不是说出口的台词则归「旁白」。
    ...
    for block in _group_blocks(quotes):
        ...
        for ctx_idxs, new_idxs in _subchunks(block):
            ...
            for k in new_idxs:
                ...
                # accept roster names, the narrator, or a crowd-group label (群众·…)
                if new in SKIP_NAMES or (new not in valid_names and not new.startswith("群众·")):
                    continue
                stats["reviewed"] += 1
