...
3. speaker 只填角色名、”旁白”或”UNKNOWN”（见规则 6）。
4. 不要输出任何解释、注释、markdown，只输出 JSON。
5. 切分粒度：以”一段旁白叙述”或”一轮对白”为基本单位。相邻旁白叙述可合并为 1-3 句一段，但绝不允许省略或丢弃任何旁白内容。
6. 当无法可靠判断说话人时，speaker 填 “UNKNOWN”，不要硬猜成已知角色名；UNKNOWN 会保留给后续人工复核。
7. 对白与旁白必须分属不同的 segment。引号内的对白归角色，引号外的叙述（含动作、表情、场景、心理描写、”说道/问道/答道/苦笑/点头”等提示语）归”旁白”。
8. 遇到 “「对白。」某某说道。” 必须拆成两段：某某 “「对白。」” + 旁白 “某某说道。”；遇到 “某某说道：「对白。」” 必须拆成：旁白 “某某说道：” + 某某 “「对白。」”。
9. 只有当单段明显过长时，才继续往下细分。
10. **严禁省略任何原文内容**：所有 segments 的 text 按顺序拼接起来必须覆盖原文的每一个字。
11. 判断 speaker 时务必结合上下文语义：
...
13. speaker 判断要优先找明确提示语和上下文轮次，不要只因为某个角色名离对白最近就归给该角色。无法高置信度判断时填 UNKNOWN。
""".strip()
...
规则：
1. 只检查和修正 speaker，必要时可把明显的动作叙述改为旁白；绝对不修改 text 字段。
...
3. speaker 只填角色名或”旁白”。
4. 旁白与对白必须严格拆开；引号外叙述归”旁白”，引号内对白归角色。
5. text 字段允许包含为了合成稳定性所做的必要保守优化，但不要过度润色或改写。
6. 不要输出解释、注释、markdown 或额外说明。
""".strip()
