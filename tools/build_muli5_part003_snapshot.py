#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""确定性重建第五卷 part_003 修正快照（2026-06-14）。

输入：
  - 原优化快照（外部）：生成 part_003 时实际使用的 task_snapshot（segments 1501-2250）。
  - docs/samples/第五卷/角色清单.json：规范名↔别名表（脚本块标签归一用）。
  - docs/samples/第五卷/muli_5_part_003_review_flags.json：单遍第二意见 + 人工复核 resolution。

两层修正（与正文 README §part_003 一致）：
  1) 确定性：群组/剧本格式节（『五女神的房间 其之二』）切成逐行说话人段，标签按角色清单归一。
  2) 人工复核：把 review_flags 中 resolution=="applied" 的改判按 quote_id 落到对应段。

输出：docs/samples/第五卷/muli_5_part_003_corrected_snapshot.json
用法：python tools/build_muli5_part003_snapshot.py  [--snapshot 路径]
"""
...
PART2 = slice(1500, 2250)  # 0-based segments 1501-2250
...
def main() -> None:
    ...
    flags = json.loads((OUT / "muli_5_part_003_review_flags.json").read_text(encoding="utf-8"))
    idx2qid = {i + 1: str(p2[i].get("quote_id")) for i in range(len(p2))}
    apply = {idx2qid[it["part003_index_1based"]]: it["applied_speaker"]
    ...
    snap_out = dict(full)
    snap_out.update({
        "stage": "segments", "exportedAt": "2026-06-14_part003_corrected",
        "note": "第五卷 part_003（原 segments 1501-2250）+ 确定性脚本块切分 + 人工复核改判。",
        "segments": out,
    })
    (OUT / "muli_5_part_003_corrected_snapshot.json").write_text(
        json.dumps(snap_out, ensure_ascii=False, indent=1), encoding="utf-8")
    print(f"part_003 段数 {len(p2)} → {len(out)}；人工改判落地 {applied}/{len(apply)}")
    print("写出 muli_5_part_003_corrected_snapshot.json")
