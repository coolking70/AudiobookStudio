...
| A1 密集场景路由到强模型 | 部分达成（效果验收失败） | API、SSE 与 UI 接入已完成。真实 `muli4_seg8` 测试中，阿里云 `qwen3.5-35b-a3b` 的 SPC 路由将整体 84.40% 降至 78.01%、密集 82.54% 降至 75.40%（修对 3、误伤 12、649.7 秒）；Batch 与 LongCat 对照均在 1,204 秒上限内未完成。当前实验开关不得默认启用，详见 `docs/a1_dense_route_evaluation_2026-07-11.md`。 |
